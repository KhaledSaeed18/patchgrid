...
- **Every foreign key between two tenant-owned tables is composite on `orgId`** — `references: [orgId, id]`, never a bare id. PostgreSQL runs referential-integrity checks with row security *disabled*, so a single-column FK lets a correctly-tenanted row point at another tenant's row and no layer objects (ADR-0023).
...
- **Never bypass tenant scope.** `runAsTenant(orgId, fn)` points the ordinary mechanism at a known org and is *not* a bypass; `runAsPlatform(fn)` runs with no tenant context and can therefore only reach platform-class tables. Import paths are restricted by module (ADR-0022) — check the ESLint rule, don't count call sites.
...
- Uniqueness is per-org for ticket numbers, team names, asset serials and KB slugs. **Category names are unique per *parent*** — a real taxonomy has `Hardware → Other` and `Software → Other`. Only `User.email` and `Organization.slug` are globally unique.
...
- **Reads return `404`, actions return `403`.** A read of an entity the actor may not see is `404`; an action on an entity they *can* see but may not perform is `403`. Nothing is left to taste.
